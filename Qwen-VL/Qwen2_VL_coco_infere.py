...
model_path = "./share_models/Qwen2-VL-2B-Instruct_GRPO_coco_8_cate_4_shot"  # after RL
ori_processor_path = "./share_models/Qwen2-VL-2B-Instruct"


def run(rank, world_size):
    torch.cuda.set_device(rank)  # 新增设备绑定
    model = Qwen2VLForConditionalGeneration.from_pretrained(
        model_path,
        torch_dtype=torch.bfloat16,
        attn_implementation="flash_attention_2",
        device_map=f"cuda:{rank}",  # 修改设备映射
    )
    ...
    category_ids_2_categoty = {
        item["id"]: item["name"] for item in instances_val2017["categories"]
    }
    category_2_categoty_ids = {
        item["name"]: item["id"] for item in instances_val2017["categories"]
    }
    ...
    split_images = instances_val2017["images"][
        int(rank * split_length) : int((rank + 1) * split_length)
    ]
    ...
    for image in tqdm(split_images):
        ...
        image_path = (
            "./data/coco/val2017/" + image["file_name"]
        )  ### Modify according to your own image path.
            ...
            try:
                ...
                try:
                    with torch.no_grad():  # Add no_grad to reduce memory usage
                        # Add generation parameters to avoid getting stuck
                        generated_ids = model.generate(
                            **inputs,
                            generation_config=generation_config,
                        )

                    # Rest of your code for processing generated_ids...
                    generated_ids_trimmed = [
                        out_ids[len(in_ids) :]
                        for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
                    ]
                ...
                try:
                    ...
                    if bbox_list == None or type(bbox_list[0]) == str:
                        ...
                        for bbox in bbox_list:
                            temp_bbox = trans_bbox(
                                image_height, image_width, bbox["Position"]
                            )
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        ### save path
        with open("prediction_results_coco_8_cate_4_shot.json", "w") as json_file:
            json.dump(global_results, json_file)
        logger.info("Done")
        logger.info("Finished running")
    else:
